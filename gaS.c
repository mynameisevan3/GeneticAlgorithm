...
int main( int argc, char *argv[] ) {
  ...
  if( argc != 6 ) {
    ...
    n               = atoi( argv[1] );  // Takes n as that is specifically mentioned in the assignment...
    POPULATION_SIZE = n;                // ...and sets it to POPULATION_SIZE so other items work correctly.
    maxGen          = atoi( argv[2] );
    pc              = atof( argv[3] );
    pm              = atof( argv[4] );
    display         = atoi( argv[5] );
